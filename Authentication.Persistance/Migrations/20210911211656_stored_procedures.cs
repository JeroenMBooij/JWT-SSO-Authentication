    ...
    public partial class stored_procedures : Migration
    {
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            var spDomains = @$"CREATE PROCEDURE [dbo].[{SPName.InsertApplication}]
                ...
                @DomainApplicationId nvarchar(max),
                @Name nvarchar(max),
                ...
                @Created datetime2(7),
                @DomainCreated datetime2(7),
                @LastModified datetime2(7),
                @DomainLastModified datetime2(7)
            AS
            BEGIN
                SET NOCOUNT ON;

                INSERT INTO dbo.Applications
                VALUES (@Id, @Name, @AdminId, @LogoLocation, @Created, @LastModified);

                INSERT INTO dbo.DomainName
                VALUES(@DomainId, @DomainName, @DomainUrl, @DomainApplicationId, @DomainCreated, @DomainLastModified)
